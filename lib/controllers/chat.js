...
//        racer_photo_small: 'img/no_racer_photo_mini.png' } ],
  ...
  if (req.body.standings.length >= 1) {
    ...
    console.log('standings loop', req.body.standings.length, req.body.standings)
    Racer.remove({});

    Racer.create(req.body.standings, function(err){
      if (err){
        console.log('err', err);
      }
    })
    ...
    //   Racer.findOneAndUpdate({'user' : itemToAdd.racer_name}, 
    //     {
