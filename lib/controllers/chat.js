...
//        racer_photo_small: 'img/no_racer_photo_mini.png' } ],
  ...
  if (req.body.standings.length >= 1) {
    ...
    for (var i = 0; i < standlen; i ++){
      var itemToAdd = {
        ...
        image : req.body.racer_photo_large,
      ...
      Racer.findOne({racer_number : itemToAdd.racer_number}, function (err, racer){
        if (err) {
          console.log('racer err', err);
        }
        if (racer){
          console.log('racer update!')
          ...
          console.log('racer Saved!')
          var newRacer = new Racer(itemToAdd);
          newRacer.save(function(err) {
            if (err) {
              // console.log('err', err);
              return res.json(400, err);
            } else{
  ...
  if (req.body.events.length >= 1) {
    ...
    for (var i = 0; i < len; i ++){
      ...
      // console.log('hello itemToAdd ', itemToAdd);
